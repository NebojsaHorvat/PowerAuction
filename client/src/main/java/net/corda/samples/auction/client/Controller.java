...
public class Controller {
    ...
    @GetMapping("history")
    public APIResponse<List<StateAndRef<AuctionState>>> getAuctionHistory() {
        try {
            QueryCriteria generalCriteria = new VaultQueryCriteria(Vault.StateStatus.CONSUMED);

            List<StateAndRef<AuctionState>> consumedAuctionList = activeParty.vaultQueryByCriteria(
                    generalCriteria,
                    AuctionState.class).getStates();

            return APIResponse.success(consumedAuctionList);
        } catch (Exception e) {
            return APIResponse.error(e.getMessage());
        }
    }
