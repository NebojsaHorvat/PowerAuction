...
run_spring_server_on_remote_machine() {
    local remote_host=("$1")
    ssh $remote_host  "docker run  --network="host"  nhtacc/corda-energy-proxy:${experiment_name}" &
}
...
echo "Sleep for 100 seconds and wait for nodes to startup on remote machines"
sleep 100s
